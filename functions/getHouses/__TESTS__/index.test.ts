...
describe('get houses handler', async () => {
  ...
  it('should get houses', async () => {
    ...
    ddbMock.on(QueryCommand, {
      TableName: 'houses', KeyConditionExpression: "#hk = :landlord", ExpressionAttributeNames: {
        "#hk": "landlord",
      }
    }).resolves({
      Items: [
    ...
    if (result) {
      ...
      expect(result!.body).toBe('{"houses":[{"houseId":"house#clhqcayfg000008mb78ug1z0t"}]}')
  ...
  it('should get houses from prod', async () => {
    ...
    ddbMock.on(QueryCommand, {
      TableName: 'houses-prod', KeyConditionExpression: "#hk = :landlord", ExpressionAttributeNames: {
        "#hk": "landlord",
      }
    }).resolves({
      Items: [
  ...
  it('handles not houses found', async () => {
    expect.assertions(4);
    ddbMock.on(QueryCommand, {
      TableName: 'INVALID_TABLE', KeyConditionExpression: "#hk = :landlord", ExpressionAttributeNames: {
        "#hk": "landlord",
      }
    }).resolves({
      Items: [
  ...
  it('handles db errors', async () => {
    ...
    if (result) {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Unexpected error while getting the houses, Error: Something wrong happened")
    }
  })
})