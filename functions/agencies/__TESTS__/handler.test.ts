...
const mockDB = (tableName: string, rejects = false, item = null) => {
  if (rejects) {
    return ddbMock
      .on(PutCommand, { TableName: tableName })
...
describe("agencies handler", () => {
  ...
  describe("Success", () => {
    it("Should execute creation correctly", async () => {
      ...
      expect(JSON.parse(response.body).response.agencyId).toBe(
        `${date.getTime()}-${mockedBodyRequest.ownerId}`,
      );
    });
    ...
    it("Should execute creation correctly when is prod", async () => {
      ...
      const event = getMockedEvent(
        JSON.stringify(mockedBodyRequest),
        Stage.PROD,
      );
  ...
  describe("Error: Bad request", () => {
    ...
    it("Should return status 400 if action is not recognized", async () => {
      ...
      expect(response.statusCode).toBe(StatusCodes.badRequest);
      expect(JSON.parse(response.body).response.errorMessage).toBe(
        MaxfriseErrorCodes.unrecognizedAction.message,
      );
    });
  });
